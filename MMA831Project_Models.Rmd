...
# load data
```{r}
train <- read.csv("/System/Volumes/Data/Users/Michael/Library/CloudStorage/iCloud Drive/MMA/MMA 831/allstate-purchase-prediction-challenge/train.csv")

test <- read.csv("/System/Volumes/Data/Users/Michael/Library/CloudStorage/iCloud Drive/MMA/MMA 831/allstate-purchase-prediction-challenge/test_v2.csv")
```
...
column_type <- c('character',rep("integer", 3), "character", rep("character", 2),
                "integer", "factor", "integer", "factor", rep("integer", 3),
                rep("factor", 2), "integer", rep("factor", 7), "integer")
# reload the dataset
train <- read.csv("/System/Volumes/Data/Users/Michael/Library/CloudStorage/iCloud Drive/MMA/MMA 831/allstate-purchase-prediction-challenge/train.csv", colClasses=column_type)

test <- read.csv("/System/Volumes/Data/Users/Michael/Library/CloudStorage/iCloud Drive/MMA/MMA 831/allstate-purchase-prediction-challenge/test_v2.csv", colClasses=column_type)
```
...
```{r}
# Function for clean_data and feature engineering
...
for(i in c('A','B','C','D','E','F','G')){
  for(k in c('A','B','C','D','E','F','G')){
    if(i!=k){
      ...
      print(ggplot(trainsub, aes(trainsub[[i]])) + geom_bar() + facet_grid(. ~ trainsub[[k]])+labs(x=paste0("Customer Selection of Option ",i),y="Frequency", title=paste0("Customer selection of Option ",i," based on their selection for Option ",k)))
    }
  }
}
...
# C_names <- list("1"="C=1", "2"="C=2", "3"="C=3", "4"="C=4")
# C_labeller <- function(variable, value){ return(C_names[value]) }
# ggplot(trainsub, aes(as.symbol(i))) + geom_bar() + facet_grid(. ~ C,
...
write.csv(pred, file="/System/Volumes/Data/Users/Michael/Library/CloudStorage/iCloud Drive/MMA/MMA 831/allstate-purchase-prediction-challenge/submit1.csv", row.names=FALSE, quote=FALSE)
...
confusionMatrix(table(rf.pred, trainex2$changed), "Yes")
```
